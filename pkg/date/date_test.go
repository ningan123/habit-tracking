...
func TestGetDateDetails(t *testing.T) {
	...
	testCases := []struct {
		...
		// 2024年年初8天
		{"2024-01-01", 2024, 1, time.Monday, ""},     // 周一
	...
	for _, tc := range testCases {
		year, weekNum, weekday, err := GetDateDetails(tc.inputDate)
		...
		// 检查结果
		if year != tc.expectedYear || weekNum != tc.expectedWeekNum || weekday != tc.expectedWeekday {
