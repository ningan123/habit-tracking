...
func DayEyeMaskHtmlTable(w http.ResponseWriter) {
	...
	// 遍历数据并构造表格的行  
	for _, item := range hData.GlobalEyeMask.DayOrderInfo { 
		if item == nil {
		  continue
		}
...
func WeekEyeMaskHtmlTable(w http.ResponseWriter) {
	// 构造HTML表格的开头  
	fmt.Fprintf(w, "<html>\n")  
	fmt.Fprintf(w, "<head>\n")  
	fmt.Fprintf(w, "<title>MyStruct Table</title>\n")  
	...
	// 遍历数据并构造表格的行  
	for _, item := range hData.GlobalEyeMask.WeekOrderInfo { 
		if item == nil {
			continue 
		}  
...
func MonthEyeMaskHtmlTable(w http.ResponseWriter) {
	// 构造HTML表格的开头  
	fmt.Fprintf(w, "<html>\n")  
	fmt.Fprintf(w, "<head>\n")  
	fmt.Fprintf(w, "<title>MyStruct Table</title>\n")  
	...
	// 遍历数据并构造表格的行  
	for _, item := range hData.GlobalEyeMask.MonthOrderInfo { 
		if item == nil {
			continue 
		} 
		...
		// for content, conEyeMaskTime := range item.EyeMaskTimeOfDifferentContent {
		// 	fmt.Fprintf(w, "<tr>")  
		// 	fmt.Fprintf(w, "<td>%d</td>", item.MonthNum)
		// 	fmt.Fprintf(w, "<td>%s</td>", item.EyeMaskTime)  
		// 	fmt.Fprintf(w, "<td>%s</td>", content)  
		// 	fmt.Fprintf(w, "<td>%s</td>", conEyeMaskTime)  
		// 	fmt.Fprintf(w, "</tr>\n")  
		// }		
	}  
...
func YearEyeMaskHtmlTable(w http.ResponseWriter) {
	// 构造HTML表格的开头  
	fmt.Fprintf(w, "<html>\n")  
	fmt.Fprintf(w, "<head>\n")  
	fmt.Fprintf(w, "<title>MyStruct Table</title>\n")  
	...
	// 遍历数据并构造表格的行  
	for _, item := range hData.GlobalEyeMask.YearOrderInfo { 
		if item == nil {
			continue
		} 
		fmt.Fprintf(w, "<tr>")  
		...
		// 	fmt.Fprintf(w, "<td>%s</td>", item.YearEyeMaskTime)  
		// 	fmt.Fprintf(w, "<td>%s</td>", content)  
		// 	fmt.Fprintf(w, "<td>%s</td>", conEyeMaskTime)  
		// 	fmt.Fprintf(w, "</tr>\n")  
		// }		
